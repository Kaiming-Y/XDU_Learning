##### 预剪枝（prepruning）和后剪枝（postpruning）后生成的决策树在测试集上的分类准确率
...
if __name__ == '__main__':
    ...
    print('剪枝前的决策树:')
    print(dicision_Tree)
    print('剪枝前的测试集分类准确率: {}'.format(predict_more(dicision_Tree, test_data, test_data_label)))

    print('-'*20  + '剪枝' + '-'*20)
    new_tree = post_prunning(dicision_Tree,test_data , test_data_label , names)
    print('剪枝后的决策树:')
    print(new_tree)
    print('剪枝后的测试集分类准确率: {}'.format(predict_more(new_tree, test_data, test_data_label)))
