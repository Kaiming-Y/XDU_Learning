    ...
    def write_predictions(name):
        ...
        with Path('results/score/{}.preds.txt'.format(name)).open('wb') as f:
            ...
            for golds, preds in zip(golds_gen, preds_gen):
                ...
                tag_bytes = tag  # tag is already bytes
                preds_bytes = preds['labels']  # preds['labels'] is already bytes
                words_bytes = b' '.join(words)  # words are already bytes and joined with space
                line = b' '.join([tag_bytes, preds_bytes, words_bytes]) + b'\n'
                f.write(line)
